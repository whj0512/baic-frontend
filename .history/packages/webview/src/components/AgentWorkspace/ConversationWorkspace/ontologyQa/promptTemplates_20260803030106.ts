...
export const ONTOLOGY_QA_QUICK_PROMPTS: readonly ConversationQuickPrompt[] = [
  ...
  },
  {
    ...
    label: '场景 9 · 查询推理结果',
    description: '调用查询 Skill，读取场景 9 已输出到项目目录的结果。',
    requiresAuthorization: false,
  },
  {
    id: 'ontology-scene-10',
    label: '场景 10 · 只读关系查询',
    description: '生成保留功能名占位符的只读查询草稿。',
    requiresAuthorization: false,
  },
]
...
export function createOntologyScene10Prompt(): string {
  return `请查询 GraphDB 仓库 requirement 中与 <功能名> 相关的关系。
只读，不上传、不推理、不修改仓库。
请按 JSON 导出关系类型、起点、终点、证据和置信度，并给出统计。
结果输出至项目 <绝对项目目录> 下的 requirement-ontology-qa.json 文件中。`
}
