...
function ReqRelationShip({
  ...
  const runReplaceQuery = useCallback(async (request: GraphDBGraphRequest) => {
    ...
    try {
      const isOverviewRequest = !request.root
      const discoveryResponse = isOverviewRequest
        ? await fetchGraphDBGraph(DEFAULT_GRAPH_REQUEST, controller.signal)
        : null

      const overviewRoots = discoveryResponse
        ? getRequirementRootIds(discoveryResponse.nodes)
        : []
      const responses = discoveryResponse
        ? await mapWithConcurrency(
            overviewRoots,
            ROOT_QUERY_CONCURRENCY,
            root => fetchGraphDBGraph({ ...request, root }, controller.signal),
          )
        : [await fetchGraphDBGraph(request, controller.signal)]
      if (requestSequence !== replaceRequestSequenceRef.current) return

      const canonicalRoot = isOverviewRequest
        ? null
        : responses[0]?.meta.root || request.root || null
      ...
      const nextGraphData = responses.reduce<GraphData>(
        (merged, response) => mergeGraphData(merged, buildReadableGraphData(response)),
        { nodes: [], edges: [] },
      )
      const nextMeta = discoveryResponse
        ? buildMergedGraphMeta(request, discoveryResponse, responses, nextGraphData)
        : responses[0]?.meta
      if (!nextMeta) {
        throw new Error('GraphDB 未返回关系图查询结果')
      }
      expandedRootsRef.current = nextExpandedRoots
      graphDataRef.current = nextGraphData
      setGraphData(nextGraphData)
      setLatestMeta(nextMeta)
      setFocusNode(canonicalRoot)
      setLayoutRevision((current) => current + 1)
      setExpandedRootCount(nextExpandedRoots.size)
      setIsTruncated(isMetaTruncated(nextMeta))
    } catch (error: unknown) {
  ...
  const handleRefresh = useCallback((requestOverrides?: Partial<GraphDBGraphRequest>) => {
    ...
    void runReplaceQuery({
      ...baseGraphRequest,
      root,
      depth: requestOverrides?.depth ?? draftDepth,
      origin: requestOverrides?.origin ?? originFilter,
    })
  }, [baseGraphRequest, draftDepth, originFilter, runReplaceQuery])
...
function getRequirementRootIds(nodes: NonNullable<GraphData['nodes']>) {
