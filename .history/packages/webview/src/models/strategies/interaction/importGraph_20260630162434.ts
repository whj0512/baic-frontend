...
type EdgeGeometry = InteractionLayout & {
    vertices?: Point[]
    router?: null
    connector?: {
        name: string
...
const convertEdge = (interaction: Interaction, geometry?: EdgeGeometry): GraphCell => {
    ...
    const cell: GraphCell = {
        ...
        shape: 'edge',
        zIndex: EDGE_Z_INDEX,
        source: geometry?.source ?? interaction.source,
        target: geometry?.target ?? interaction.target,
        labels,
        attrs: {
            line: {
                stroke: '#1890ff',
    ...
    if (geometry?.vertices) {
        cell.vertices = geometry.vertices
    }
    if (geometry?.router !== undefined) {
        cell.router = geometry.router
    }
    if (geometry?.connector) {
        cell.connector = geometry.connector
    }

    return cell
}

const createInteractionEdgeGeometry = (
    senderPos: ComponentLayout,
    receiverPos: ComponentLayout,
    edgeY: number,
    isSelfLoop: boolean
): EdgeGeometry => {
    if (isSelfLoop) {
        const rightX = senderPos.x + senderPos.width
        return {
            source: { x: rightX, y: edgeY },
            target: { x: rightX, y: edgeY + SELF_LOOP_HEIGHT },
            vertices: [
                { x: rightX + SELF_LOOP_WIDTH, y: edgeY },
                { x: rightX + SELF_LOOP_WIDTH, y: edgeY + SELF_LOOP_HEIGHT },
            ],
            router: null,
            connector: {
                name: 'rounded',
                args: { radius: EDGE_CONNECTOR_RADIUS },
            },
        }
    }
    ...
    return {
        source: {
            x: senderPos.x + senderPos.width / 2,
            y: edgeY,
        },
        target: {
            x: receiverPos.x + receiverPos.width / 2,
            y: edgeY,
        },
    }
}
...
export const importGraphFromJSON = (jsonString: string): { cells: GraphCell[] } => {
    ...
    if (apiData.interactions && Array.isArray(apiData.interactions)) {
        apiData.interactions.forEach((interaction, edgeIndex) => {
            ...
            const edgeGeometry = createInteractionEdgeGeometry(senderPos, receiverPos, edgeY, senderId === receiverId)

            const repositionedInteraction: Interaction = {
                ...interaction,
                source: edgeGeometry.source,
                target: edgeGeometry.target,
            }
            ...
            cells.push(convertEdge(repositionedInteraction, edgeGeometry))
        })
    }
