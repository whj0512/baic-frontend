...
- 身份认证与请求头（重要）：
  - Authorization: Bearer <token>
    ...
    - 错误：缺失或无效 token 将返回 HTTP 401（{"detail": "Missing or invalid Authorization header"} 或 {"detail":"Invalid or expired token"}）。
  - Content-Type
  ...
  - WebSocket 认证
    ...
    - 服务端会在连接建立时解码并验证该 token；若 token 无效，连接会被拒绝（close code 1008，reason: "Invalid or expired token"）。
