...
export const ONTOLOGY_QA_QUICK_PROMPTS: readonly ConversationQuickPrompt[] = [
  ...
  {
    id: 'ontology-scene-10-results',
    label: '场景 10 · 查询功能关系结果',
    description: '填写功能名，调用查询 Skill 读取场景 10 已输出的关系结果。',
    requiresAuthorization: false,
  },
]
...
结果输出至项目 <绝对项目目录> 下的 ${functionName}-relation.json 文件中。`
}

export function createOntologyScene10ResultsQueryPrompt(
  functionName: string,
): string {
  return `$query-project-function-relations project_root="<绝对项目目录>" function_name="${functionName}"`
}
