...
const STREAM_DELAY_MIN_MS = 10_000
const STREAM_DELAY_MAX_MS = 30_000

function getRandomStreamDelay(): number {
  return Math.floor(
    Math.random() * (STREAM_DELAY_MAX_MS - STREAM_DELAY_MIN_MS + 1),
  ) + STREAM_DELAY_MIN_MS
}
...
function buildIngestionReply(
  ...
  if (stepIndex === 9) {
    return {
      text: [
        ...
        '',
        '已回放历史流程中的上传确认与校验摘要。本次不会执行真实上传、清库、覆盖 TBox/SHACL 或推理操作。',
  ...
  return {
    text: '**流程已完成。** 请点击“新建对话”从文档条目化重新开始。',
  }
}
...
function buildQaReply(snapshot: DemoSnapshot, stepIndex: number): DemoReply {
  ...
  if (stepIndex === 0) {
    return {
      text: [
        ...
        '本次未连接 GraphDB 或执行推理；下一步可查看完整结构化结果。',
      ].join('\n'),
    }
  }
  if (stepIndex === 1) {
  ...
  return {
    text: '**流程已完成。** 请点击“新建对话”重新开始场景 9/10 回放。',
  }
}
...
async function* streamDemoChat(
  ...
  const streamDelayMs = getRandomStreamDelay()
  const emit = async (event: QwenPawSseEvent) => {
    await delay(streamDelayMs, signal)
    onActivity?.()
    return event
  }
