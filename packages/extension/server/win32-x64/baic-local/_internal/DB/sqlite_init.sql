...
DROP TABLE IF EXISTS req_requirement_model;
...
CREATE TABLE req_requirement_model (
  id TEXT PRIMARY KEY,
  ...
  source_representation TEXT NOT NULL DEFAULT 'both',
  context_model_group_id TEXT,
  ...
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (requirement_version_id) REFERENCES req_requirement(id) ON DELETE CASCADE,
  UNIQUE (requirement_version_id, dimension_code, model_key)
);

CREATE INDEX idx_req_requirement_model_version
  ON req_requirement_model(requirement_version_id);
CREATE INDEX idx_req_requirement_model_group
  ON req_requirement_model(model_group_id);
CREATE INDEX idx_req_requirement_model_requirement
  ON req_requirement_model(requirement_group_id, dimension_code);
CREATE UNIQUE INDEX idx_req_requirement_model_primary
  ON req_requirement_model(requirement_version_id, dimension_code)
  WHERE is_primary = 1;
